...
- [x] Worth a look is a distinct finding type. It carries a Source sentence (the clause is present in
...
- [x] Entries are written flatly: what the clause does and that it is bounded ("liability is capped
      at 2× fees"). They are never written as hedges ("this may be worth reviewing"), because bounded
      is a fact about the clause, not a confidence level. (Enforced in the prompt; only a real-model
      run can show whether the model obeys.)
- [x] Worth a look is shown separately from Risk flags and collapsed by default.
- [x] Test at the analysis seam, synthetic client: a clause with a stated liability cap comes back as
