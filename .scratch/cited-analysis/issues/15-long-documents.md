...
- [x] A Document longer than the model's context window is analysed end to end, and no part is
      silently skipped. (Default part budget 60,000 characters of unit lines; the model's real
      window is unknown in code because no model id is hardcoded.)
- [x] Spans from any part resolve to offsets in the full stored text and pass verbatim validation.
- [x] Missing protections and Nice to have items are raised only when the whole Document lacks the
      term.
- [x] The summary, Worth a look and Multiplier notes behave correctly across parts.
- [x] If any part fails, the whole analysis reports failure, never a partial result.
- [x] Tests at the analysis seam, synthetic client: a Risk flag planted in the final pages of a long
